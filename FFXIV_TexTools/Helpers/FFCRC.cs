...
namespace FFXIV_TexTools.Helpers
{
    ...
    class FFCRC
    {
        ...
        uint[] crc_table_0f089d0 = new uint[]{
        ...
        uint[] crc_table_0f08dd0 = new uint[]{
        ...
        uint[] crc_table_0f091d0 = new uint[]{
        ...
        public int ComputeCRC(byte[] ecxBytes)
        {
            return ComputeCRC(ecxBytes, 0, ecxBytes.Length);
        }

        public int ComputeCRC(byte[] ecxBytes, int offset)
        {
            ...
            using (MemoryStream stream = new MemoryStream(ecxBytes))
            {
                using (BinaryReader reader = new BinaryReader(stream))
                {
                    ...
                    for (int i = 0; i < cbEndUnalignedBytes; ++i)
                    {
                        ...
                        {
                            dwCRC = unchecked((int)(crc_table_0f085d0[(dwCRC ^ b) & 0x000000FF] ^ (dwCRC >> 8)));
                        }
                    }
                }
            }
